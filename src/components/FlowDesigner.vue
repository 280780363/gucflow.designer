  ...
  <div>
    ...
    <div id="container" :style="{width:paperWidth+'px',height:paperHeight+'px',position:'relative'}">
      ...
      <svg :style="{position:'absolute','z-index':10,top:0,left:0}" :height="paperHeight" version="1.1" :width="paperWidth" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" @mousemove="moving($event)" @mouseup="drop($event)">
        ...
        <g v-for="item in nodes" :key="'node'+item.id" :id="item.id" class="pointer" @dblclick="nodeDblClick(item)" @mousedown="beginMove($event)">
        ...
        <g v-for="item in lines" :key="'line'+item.id" class="pointer" @dblclick="lineDblClick(item)">
